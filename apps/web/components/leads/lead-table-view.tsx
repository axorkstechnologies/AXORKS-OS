...
export function LeadTableView({ leads, onVerifyLeads }: LeadTableViewProps) {
  ...
  if (leads.length === 0) {
    return (
      <div className="text-center py-12 text-slate-300 text-xs rounded-2xl border border-slate-800 bg-slate-950/80">
  ...
  return (
    <div className="space-y-3">
      ...
      {selectedIds.size > 0 && (
        <div className="p-3.5 rounded-2xl border border-violet-500/50 bg-violet-950/40 flex items-center justify-between gap-3 animate-in fade-in slide-in-from-top-2 duration-150 shadow-xl">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl border border-slate-700 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold transition shadow-xs"
            >
              Clear Selection
            </button>
            ...
            <button
              ...
              className="px-4 py-1.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white text-xs font-black shadow-md shadow-violet-600/40 transition flex items-center gap-1.5"
            >
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950/90 shadow-xl">
        <table className="w-full text-left text-xs">
          <thead className="border-b border-slate-800 bg-slate-900/90 text-slate-200 font-bold uppercase tracking-wider text-[11px]">
            <tr>
              ...
              return (
                ...
                >
                  ...
                  <td className="p-3.5">
                    <Link
                      href={`/leads/${lead.id}`}
                      className="font-bold text-white group-hover:text-violet-300 transition-colors text-sm"
                    >
                    ...
                    {lead.website && (
                      <div className="text-[11px] text-slate-300 flex items-center gap-1 mt-0.5 font-medium">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="hover:underline hover:text-white truncate max-w-xs"
                  ...
                  <td className="p-3.5">
                    ...
                    <div className="text-[11px] text-slate-300 truncate max-w-xs font-mono">
                      {lead.decision_maker_title || lead.email || ""}
                    </div>
                  </td>

                  <td className="p-3.5">
                    <span className="inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-violet-500/20 text-violet-300 border border-violet-500/40">
                      {lead.status || "new"}
                    </span>
                  </td>
                  ...
                  <td className="p-3.5 text-slate-300 capitalize text-xs font-medium">
                    {lead.source || "manual"}
                  </td>
                  ...
                  <td className="p-3.5 text-right space-x-1.5">
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg bg-violet-600/25 hover:bg-violet-600 text-violet-200 hover:text-white border border-violet-500/40 transition text-xs font-bold inline-flex items-center gap-1 shadow-xs"
                    ...
                    <Link
                      href={`/leads/${lead.id}`}
                      className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white border border-slate-700 transition text-xs font-bold inline-block shadow-xs"
                    >
                      View
                    </Link>
                  </td>
